...
async function push(collectionId, itemId, propertyName, value, options) {
    try {
        ...
        const item = await collection.findOneAndUpdate({ _id: (0, item_helpers_1.convertStringId)(itemId) }, { $push: { [editedModify.propertyName]: editedModify.value } }, { readConcern: consistentRead === true ? "majority" : "local", returnDocument: "after", includeResultMetadata: false });
        if (item) {
            if (suppressHooks != true) {
