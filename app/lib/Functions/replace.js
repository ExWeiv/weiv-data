...
async function replace(collectionId, item, options) {
    try {
        ...
        const value = await collection.findOneAndReplace({ _id: itemId }, { ...replaceItem, _updatedDate: new Date() }, { readConcern, returnDocument: "after", includeResultMetadata: false });
        if (value) {
            if (suppressHooks != true) {
